// Cache buster - Force browser to recognize this is a new version
console.log('Checkout-fixed.js loaded - Version 3.0 - Cache bypassed completely');
...
// CRITICAL: Define updateCartUI function IMMEDIATELY to prevent any ReferenceError
// This must be the very first function defined to ensure it's available
...
// Also define as regular function
...
// Expose functions to global scope immediately after definition
...
async function handleDirectOrder() {
    try {
        ...
        if (result.cartCleared) {
            console.log('Cart was cleared on server, will update UI after order completion');
        }
...
// Expose placeOrder and verifyPayment to global scope
