...
exports.createCoupon = async (req, res) => {
    try {
        const {
            ...
            minPurchase, maxDiscount, validFrom, validUntil, usageLimit
        } = req.body;
    ...
    try {
        ...
        if (!coupon.isActive) {
            if (coupon.validUntil < now) {
                return res.status(400).json({ success: false, message: 'Cannot enable expired coupon' });
            }
            if (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit) {
                return res.status(400).json({ success: false, message: 'Cannot enable coupon - usage limit reached' });
            }
        }
...
exports.checkCouponCode = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to check coupon code' });
    }
};
